...
function pintaArticulos(orden){
	...
	listaArticulosCopy.forEach(art=>{
		...
		<div class='col'>
			<div class='card mb-4'>
				<img src='assets/${art.id}.jpg' class='card-img-top'>
				<div class='card-body'>
				...
				<button id='${art.id}' class='btn-success m-3 rounded'>Comprar</button>
			</div>
		</div> 
		`
	})
	...
	btns.forEach(btn=>{
		btn.addEventListener("click",()=>{
			cart.anyadeArticulo(listaArticulos.find(art => art.id == btn.id)) //Lo mete en la clasew carrito
...
function ponArticuloEnCarrito(){
	...
	cart.articulos.forEach(cartArt=>{
		...
		cesta += `
		<tr>
			<td class="oculto">${cartArt.id}</td>
			<td><img style="width:50px;" src="assets/${cartArt.id}.jpg"></td>
			...
			<td class="${cartArt.id}" art="${cartArt.id}">
